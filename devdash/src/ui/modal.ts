import type { SuccessState, DetailsState } from '../types';
 ...
 */
export function renderModalStatusBarHtml(detailsState: DetailsState): string {
  if (detailsState.status === 'revalidating') {
    return `
...
/**
 * Renders the detailed view of a product structure.
 */
function renderProductDetailsHtml(product: Product): string {
...
/**
 * Renders only the inner body HTML of the modal using exhaustive type checks.
 */
export function renderModalBodyHtml(detailsState: DetailsState): string {
  switch (detailsState.status) {
    ...
    case 'error':
      if (detailsState.data) {
        return renderProductDetailsHtml(detailsState.data);
      }
      return `
        <div class="text-center py-10 px-5">
          ...
          <p class="text-slate-500 dark:text-slate-400 text-sm mb-6">${detailsState.errorMessage}</p>
    ...
    default:
      return assertNever(detailsState);
  }
}
...
export function renderModalContent(state: SuccessState): string {
  ...
  const bodyHtml = renderModalBodyHtml(state.details);
  const statusBarHtml = renderModalStatusBarHtml(state.details);
